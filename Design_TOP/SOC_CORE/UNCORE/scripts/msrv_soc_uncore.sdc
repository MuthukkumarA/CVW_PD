...
set ip [remove_from_collection [all_inputs] [get_ports HCLK]]

set ip1 [remove_from_collection [all_inputs] [get_ports PCLK]]

 
if {[shell_is_in_topographical_mode]} {
  create_scenario ${scenario}
  create_clock -period 4 -name MAIN [get_ports HCLK]
  ...
  create_clock -name MAIN -period 4 [get_ports HCLK]
  set_input_delay 0.75 -clock MAIN $ip
  set_output_delay -clock MAIN 0.75 [get_ports [all_outputs]]
	
  create_clock -name PCLK -period 6 [get_ports PCLK]
  set_input_delay 0.75 -clock PCLK $ip1
  set_output_delay -clock PCLK 0.75 [get_ports [all_outputs]]
...
set_operating_conditions -min ss0p585vm40c -max ss0p585v125c
